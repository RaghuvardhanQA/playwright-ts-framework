...
Tests in this project use utility functions from `#utils/*` (ActionUtils, ElementUtils, ExpectUtils, PageUtils).
When fixing tests, use these utilities instead of raw Playwright API calls.

## Browser Strategy

Follow the tiered approach:
...
2. **If the user describes the failure by feature** (e.g., "fix the login test"):
   - `Grep` for the feature keyword in `src/tests/specs/**/*.spec.ts`
   - `Grep` in `src/tests/pages/` for related page objects
...
3. **Error Investigation**: Use available tools to diagnose:
   ...
   - Read test source and page objects with `Read` and `Grep`
...
5. **Code Remediation**: Edit the test code using `Edit` tool, applying project utility patterns:

   | Instead of (raw Playwright) | Use (project utils) |
   |---|---|
...
- Use project utility functions from `#utils/*` for all test code
- Ensure tests import `test` from `#pagesetup` (which calls `setPage(page)` automatically)

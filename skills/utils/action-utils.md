...
Source: `src/main/utils/action-utils.ts`
...
### `focus(input, options?: TimeoutOption & VisibilityOption)`
...
### `downloadFile(input, path: string, options?: ClickOptions)`

Clicks the element to trigger a download, saves to `path`. Works with remote browsers.
...
type SelectOptions = PlaywrightSelectOptions & VisibilityOption & StabilityOption;
type CheckOptions = PlaywrightCheckOptions & VisibilityOption & StabilityOption;
...
> **Rule**: Use `stable: true` option for elements that animate or transition before interaction.
